...
class LoggerPrivate
{
    public:
        ...
        QMultiMap<QString, AbstractAppender*> myCategoryAppenders;
        QStringList myNoAppendersCategories; //<! Categories without appenders that was already warned about
        QString     myDefaultCategory;
        bool        myWriteDefaultCategoryToGlobalInstance;

}; // end class LoggerPrivate
/******************************************************************************
** Global                                                                     *
*******************************************************************************/
...
} // end cleanupLoggerGlobalInstance
/******************************************************************************
** qtLoggerMessageHandler                                                     *
*******************************************************************************/
 ...
 */
Logger::Logger() : d_ptr(new LoggerPrivate)
{
    Q_D(Logger);
    d->myWriteDefaultCategoryToGlobalInstance = false;
} // end Logger
/******************************************************************************
** Logger Constructor                                                         *
*******************************************************************************/
 ...
 */
Logger::Logger(const QString &defaultCategory, bool writeToGlobalInstance) : d_ptr(new LoggerPrivate)
{
    Q_D(Logger);
    d->myWriteDefaultCategoryToGlobalInstance = writeToGlobalInstance;
...
Logger::~Logger()
{
    ...
    //deleteList.unite(QSet<AbstractAppender*>::fromList(d->myCategoryAppenders.values()));
    deleteList.unite(QSet<AbstractAppender*>(d->myCategoryAppenders.begin(), d->myCategoryAppenders.end()));
...
QString Logger::levelToString(Logger::LogLevel logLevel)
{
    switch (logLevel)
    {
        case Trace:   return QLatin1String("Trace");
        case Debug:   return QLatin1String("Debug");
        case Info:    return QLatin1String("Info");
        case Warning: return QLatin1String("Warning");
        case Error:   return QLatin1String("Error");
        case Fatal:   return QLatin1String("Fatal");
    }
...
Logger::LogLevel Logger::levelFromString(const QString &s)
{
    ...
    if      (str == QLatin1String("trace"))     result = Trace;
    else if (str == QLatin1String("debug"))     result = Debug;
    else if (str == QLatin1String("info"))      result = Info;
    else if (str == QLatin1String("warning"))   result = Warning;
    else if (str == QLatin1String("error"))     result = Error;
    else if (str == QLatin1String("fatal"))     result = Fatal;
...
void Logger::registerAppender(AbstractAppender *appender)
{
    ...
    if (!d->appenders.contains(appender)) d->appenders.append(appender);
    else                                  std::cerr << "Trying to register appender that was already registered" << std::endl;
...
void Logger::registerCategoryAppender(const QString& category, AbstractAppender* appender)
{
    ...
    if (!d->myCategoryAppenders.values().contains(appender)) d->myCategoryAppenders.insert(category, appender);
    else               std::cerr << "Trying to register appender that was already registered" << std::endl;
} // end registerCategoryAppender
/******************************************************************************
** removeAppender                                                             *
*******************************************************************************/
...
void Logger::removeAppender(AbstractAppender *appender)
{
    ...
    for (QMultiMap<QString, AbstractAppender*>::iterator it = d->myCategoryAppenders.begin(); it != d->myCategoryAppenders.end();)
    {
        if (it.value() == appender) it = d->myCategoryAppenders.erase(it);
        else                      ++it;
    }
} // end removeAppender
/******************************************************************************
...
{
    //if (myDefaultCategory != category) { }
    Q_D(Logger);

    QMutexLocker locker(&d->loggerMutex);

    d->myDefaultCategory = category;
} // end
/******************************************************************************
** defaultCategory                                                            *
*******************************************************************************/
...
} // end logToGlobalInstance
/******************************************************************************
** write                                                                      *
*******************************************************************************/
void Logger::write(const QDateTime& timeStamp, LogLevel logLevel, const char* file, int line, const char* function, const char* category, const QString& message, bool fromLocalInstance)
{
    Q_D(Logger);
    ...
    if (logCategory.isNull() && !d->myDefaultCategory.isNull()) logCategory = d->myDefaultCategory;
    ...
    {
        QList<AbstractAppender*> appenders = d->myCategoryAppenders.values(logCategory);
        if (appenders.length() == 0)
        {
            if (logCategory != d->myDefaultCategory && !linkedToGlobal && !fromLocalInstance && !d->myNoAppendersCategories.contains(logCategory))
            {
                std::cerr << "No appenders associated with category " << qPrintable(logCategory) << std::endl;
                d->myNoAppendersCategories.append(logCategory);
            }
        }
        else
        {
    ...
    if (logCategory.isNull() || logCategory == d->myDefaultCategory || linkedToGlobal)
    {
    ...
    if (!isGlobalInstance)
    {
        ...
        if (d->myWriteDefaultCategoryToGlobalInstance && logCategory == d->myDefaultCategory)
        {
...
#else
        QString result = QString(QLatin1String("[%1] <%2> %3")).arg(levelToString(logLevel), -7).arg(AbstractStringAppender::stripFunctionName(function)).arg(message);
        std::cerr << qPrintable(result) << std::endl;
#endif
    }

    if (logLevel == Logger::Fatal) abort();
} // end write
/******************************************************************************
** write                                                                      *
*******************************************************************************/
 ...
 * \sa AbstractAppender
 */
void Logger::write(const QDateTime& timeStamp, LogLevel logLevel, const char* file, int line, const char* function, const char* category, const QString& message)
{
 ...
 * \sa write()
 */
void Logger::write(LogLevel logLevel, const char* file, int line, const char* function, const char* category, const QString& message)
{
...
void LoggerTimingHelper::start(const char* msg, ...)
{
    va_list va;
    va_start(va, msg);
    myBlock = QString().vasprintf(msg, va);
    va_end(va);
...
/******************************************************************************
** ~LoggerTimingHelper Deconstructor                                          *
*******************************************************************************/
LoggerTimingHelper::~LoggerTimingHelper()
{
    QString message;
    if (myBlock.isEmpty()) message = QString(QLatin1String("Function %1 finished in ")).arg(AbstractStringAppender::stripFunctionName(myFunctionName));
    else                   message = QString(QLatin1String("\"%1\" finished in ")).arg(myBlock);

    int elapsed = myTime.elapsed();
    if (elapsed >= 10000 && myTimingMode == Logger::TimingAuto)   message += QString(QLatin1String("%1 s.")).arg(elapsed / 1000);
    else                                                          message += QString(QLatin1String("%1 ms.")).arg(elapsed);

    myLogger->write(myLogLevel, myFileName, myLine, myFunctionName, nullptr, message);
} // end ~LoggerTimingHelper
/******************************************************************************
** CuteMessageLogger                                                          *
*******************************************************************************/
/******************************************************************************
** CuteMessageLogger Constructor                                              *
*******************************************************************************/
CuteMessageLogger::CuteMessageLogger(Logger *l, Logger::LogLevel level, const char *file, int line, const char *function) : myLogger(l), myLogLevel(level), myFileName(file), myLine(line), myFunctionName(function), myCategoryName(nullptr)
{
...
** CuteMessageLogger Constructor                                              *
*******************************************************************************/
CuteMessageLogger::CuteMessageLogger(Logger *l, Logger::LogLevel level, const char *file, int line, const char *function, const char *category) : myLogger(l), myLogLevel(level), myFileName(file), myLine(line), myFunctionName(function), myCategoryName(category)
{
...
{
    myLogger->write(myLogLevel, myFileName, myLine, myFunctionName, myCategoryName, myMessage);
} // end ~CuteMessageLogger
/******************************************************************************
** write                                                                      *
*******************************************************************************/
