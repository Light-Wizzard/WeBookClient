...
#define LOG_ASSERT_X(cond, msg) ((!(cond)) ? cuteLoggerInstance()->writeAssert(__FILE__, __LINE__, Q_FUNC_INFO, msg) : qt_noop())
    ...
    #include <functional>
    #define LOG_CATEGORY(category) Logger customCuteLoggerInstance{category}; std::function<Logger*()> cuteLoggerInstance = [&customCuteLoggerInstance]() { return &customCuteLoggerInstance; };
    #define LOG_GLOBAL_CATEGORY(category) Logger customCuteLoggerInstance{category, true}; std::function<Logger*()> cuteLoggerInstance = [&customCuteLoggerInstance]() { return &customCuteLoggerInstance; };
#else
    #define LOG_CATEGORY(category) Logger* cuteLoggerInstance() { static Logger customCuteLoggerInstance(category); return &customCuteLoggerInstance; }
    #define LOG_GLOBAL_CATEGORY(category) Logger* cuteLoggerInstance() { static Logger customCuteLoggerInstance(category); customCuteLoggerInstance.logToGlobalInstance(category, true); return &customCuteLoggerInstance; }
#endif
/******************************************************************************
** class Logger                                                               *
*******************************************************************************/
...
class CUTELOGGERSHARED_EXPORT Logger
{
    ...
    public:
        Logger();
        explicit Logger(const QString &defaultCategory, bool writeToGlobalInstance = false);
        ~Logger();
    ...
    private:
        void write(const QDateTime &timeStamp, LogLevel logLevel, const char *file, int line, const char *function, const char *category, const QString &message, bool fromLocalInstance);
        Q_DECLARE_PRIVATE(Logger)
        //Q_UNUSED(d_func);
        LoggerPrivate *d_ptr; // d_ptr is defined in <QtCore/qglobal.h>
}; // end Logger
/******************************************************************************
...
#  endif
    ...
    private:
        Logger             *myLogger;
        Logger::LogLevel    myLogLevel;
        const char         *myFileName;
        int                 myLine;
        const char         *myFunctionName;
        const char         *myCategoryName;
        QString             myMessage;
...
class CUTELOGGERSHARED_EXPORT LoggerTimingHelper
{
    ...
    public:
        inline explicit LoggerTimingHelper(Logger *l, Logger::LogLevel logLevel, const char *file, int line, const char *function) : myLogger(l), myLogLevel(logLevel), myTimingMode(Logger::TimingAuto), myFileName(file), myLine(line), myFunctionName(function)
        {
...
#  endif
    ...
    private:
        ...
        Logger::LogLevel    myLogLevel;
        Logger::TimingMode  myTimingMode;
        const char         *myFileName;
        int                 myLine;
        const char         *myFunctionName;
        QString             myBlock;
