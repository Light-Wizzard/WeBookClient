...
*******************************************************************************/
WeBookMan::WeBookMan(const QStringList &strings, QObject *parent) : QAbstractListModel(parent), stringList(strings)
{
    myParent = parent;
    weBookCommon = new WeBookCommon(true);
    LOG_DEBUG() << "WeBookMan Constructor";
} // end WeBookMan
/******************************************************************************
** forEach                                                                    *
...
void WeBookMan::forEachSave(const QModelIndex &parent)
{
    ...
    for(int myRow = 0; myRow < rowCount(parent); ++myRow)
    {
        ...
        else
        {
            ...
            if (isDebugAllMessage) LOG_DEBUG() << QString("name=%1").arg(cellOne);
        }
    } // end for(int myRow
} // end forEach
/******************************************************************************
...
{
    if (isDebugMessage) LOG_DEBUG() << "Get Data";
    weBookListItemsReturned.clear();
    forEachSave(QModelIndex());
    if (isDebugMessage) LOG_DEBUG() << "TreeModel::getData()=" << weBookListItemsReturned;
...
{
    if (isDebugMessage) LOG_DEBUG() << "Get StringList";
    return stringList;
} // end getStringList
/******************************************************************************
...
{
    if (isDebugMessage) LOG_DEBUG() << "data(index, role=" << role << ")";
...
{
    if (isDebugMessage) LOG_DEBUG() << "Set Data(index, value=" << value << ", role=" << role << ")";
...
{
    if (isDebugMessage) LOG_DEBUG() << "rowCount";
    Q_UNUSED(parent);
...
{
    if (isDebugMessage) LOG_DEBUG() << "headerData";
...
{
    if (isDebugMessage) LOG_DEBUG() << "flags";
...
{
    if (isDebugMessage) LOG_DEBUG() << "insertRows";
    Q_UNUSED(parent);
...
{
    if (isDebugMessage) LOG_DEBUG() << "removeRows";
    Q_UNUSED(parent);
